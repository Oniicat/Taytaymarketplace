...
if (isset($_POST["submit"])) {
    $org = mysqli_real_escape_string($conn, $_POST['org_name']);

    $stmt = $conn->prepare("INSERT INTO `tb_org`(`org_name`, `date_added`) VALUES (?, NOW())");
    $stmt->bind_param("s", $org);

    if ($stmt->execute()) {
      header('Location: main.php?page=org');
      exit();
    } else {
      header('Location: main.php?page=org');
      exit();
...
if (isset($_POST['update'])) {
    $orgid = mysqli_real_escape_string($conn, $_POST['org_id']);
    $org = mysqli_real_escape_string($conn, $_POST['org_name']);

    $stmt = $conn->prepare("UPDATE `tb_org` SET `org_name`=? WHERE `org_id`=?");
    $stmt->bind_param("ss", $org, $orgid);
...
if (isset($_GET["org_id"])) {
    ...
    $sql = "DELETE FROM `tb_org` WHERE `org_id` = '$orgid'";
  ...
  text-decoration: none;
    ...
    <div class="container">
        ...
        <div class="search-container">
            ...
            <button id="openAddModalBtn" class="btnn" onclick="openAddModal()"><i class="fas fa-plus"></i> Add New Organization</button>
        </div>
        <table class="table table-hover text-center" id="orgTable">
            <thead>
                <tr>
            ...
            <tbody>
                <?php
                $sql = "SELECT org_id, org_name, date_added FROM `tb_org`";
                ...
                if ($result && mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<tr>
                                ...
                                <td>
                                   <button class="btn" onclick="openEditModal(\'' . $row["org_id"] . '\', \'' . htmlspecialchars($row["org_name"]) . '\')">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <a href="org.php?org_id=' . $row["org_id"] . '" class="btn" onclick="return confirm(\'Are you sure?\')">
    ...
    <div id="addorgModal" class="modal">
        <div class="modal-content">
            ...
            <h3>Add New Org</h3>
            <form action="org.php" method="post"  id="addorgForm" onsubmit="submitorgForm(event)">
                <label>Organization:</label>
                <input type="text" name="org_name"  id="addorg" required>
    ...
    <div id="editorgModal" class="modal">
        <div class="modal-content">
            ...
            <form action="" method="post">
                ...
                <label>Organization:</label>
                <input type="text" name="org_name" id="editorg" required>
    ...
    <script>
        ...
        function openEditModal(orgid, org) {
        ...
        function filterTable() {
            ...
            let table = document.getElementById("tb_org");
